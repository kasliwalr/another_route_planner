...
void Render::DrawLocations(io2d::output_surface &surface) const
{
  auto ways = m_Model.Ways().data();
  for(auto &loc: m_Model.Locations())
  {
    auto &way = ways[loc.way];
    auto path = PathFromWay(way);
    surface.stroke(m_LocationBrush, path, std::nullopt, io2d::stroke_props{m_LocationWidth}, io2d::dashes{});
  }
} 
